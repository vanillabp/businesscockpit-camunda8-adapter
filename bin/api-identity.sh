#!/usr/bin/env bash
#
# The API of every release line has to be IDENTICAL. A user must never read the version
# suffix of an artifact to find out which methods exist; where a line's cluster cannot do
# something, the method is still there and degrades with a guiding message. The VanillaBP
# Camunda 8 adapter holds itself to the same rule, and this extension sits on top of it.
#
...
#
# No cluster and no tests: this is a compile and a comparison, which is why it runs on
# every pull request while the integration tests of the other lines wait for the nightly
# matrix.
#
...
for line in "${lines[@]}"; do
  ...
  # CLEAN, always: classes compiled against another line's client stay binary compatible
  # with nothing. A method inherited in one client and declared in the other is called
  # through the owner the compiler saw, so a stale target/ turns into a
  # NoClassDefFoundError at runtime instead of a compile error.
  ...
  # it: a Quarkus test application puts a runner and a copy of every dependency into
  # target/quarkus-app, and none of that is an API of this repository
